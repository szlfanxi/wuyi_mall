...
const toggleFavorite = async (productId) => {
  try {
    if (isFavorite(productId)) {
      ...
      // 发送正确格式的 POST 请求，使用空对象作为请求体
      const res = await request.post(`/api/favorites/remove?productId=${productId}`, {})
      if (res.success) {
        favoriteProducts.value.delete(productId)
        // Show success message with animation
        showNotification('已成功取消收藏', 'success')
      } else {
        showNotification(res.message || '取消收藏失败', 'error')
      }
    } else {
      // Add to favorites
      // 发送正确格式的 POST 请求，使用空对象作为请求体
      const res = await request.post(`/api/favorites/add?productId=${productId}`, {})
      if (res.success) {
        favoriteProducts.value.add(productId)
        // Show success message with animation
        showNotification('已成功添加到收藏', 'success')
      } else {
        showNotification(res.message || '添加收藏失败', 'error')
      }
    }
  } catch (error) {
