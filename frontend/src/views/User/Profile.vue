    ...
    <div class="profile-content">
      ...
      <div class="profile-stats">
        <div class="stat-item">
          ...
          <div class="stat-label">待商家确认</div>
        </div>
        <div class="stat-item">
...
const orderStats = computed(() => {
  return {
    pending: orders.value.filter(order => order.status === 0).length, // 待商家确认
    shipped: orders.value.filter(order => order.status === 2).length, // 待发货
    delivered: orders.value.filter(order => order.status === 3).length, // 待收货
    completed: orders.value.filter(order => order.status === 4).length // 待评价
  }
})
