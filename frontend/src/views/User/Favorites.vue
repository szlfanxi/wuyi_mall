        ...
        <div v-for="favorite in filteredFavorites" :key="favorite.id" class="favorite-item">
          ...
          <div class="favorite-info">
            ...
            <div class="favorite-actions">
              ...
              <button @click="removeFavorite(favorite.id)" class="remove-btn">取消收藏</button>
            </div>
          </div>
        </div>
      </div>
...
const removeFavorite = async (favoriteId) => {
  try {
    ...
    const response = await request.post('/api/favorites/remove', { productId: favorite.productId })
    ...
    if (response.success) {
      ...
      showNotification(`取消收藏失败: ${response.message}`, 'error')
    }
  } catch (err) {
