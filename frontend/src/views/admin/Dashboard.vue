<template>
  <div class="dashboard-container">
    ...
    <div class="stats-cards">
      <div class="stat-card" @click="navigateToProducts">
        ...
        <table class="orders-table">
          ...
          <tbody>
            <tr v-for="order in recentOrders" :key="order.id">
              ...
              <td>
                <button class="view-btn" @click="viewDetail(order.id)">查看详情</button>
              </td>
            </tr>
          </tbody>
        </table>
...
const fetchStats = async () => {
  try {
    ...
    recentOrders.value = ordersResponse.map(order => {
      // 从order_item中获取receive_name，若无则使用username
      const receiveName = order.orderItems && order.orderItems.length > 0 ? order.orderItems[0].receiveName : order.username;
      return {
        id: order.id,
